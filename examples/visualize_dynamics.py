...
def generate_topology_dynamics_plot(args):
    ...
    for task_id in range(n_tasks):
        ...
        split_history.append(action.num_split)
        prune_history.append(action.num_pruned)
        merge_history.append(action.num_merged)
    ...
    axes[1, 1].set_ylimshow(accuracy_matrix, cmap='RdYlGn', vmin=0, vmax=1)
...
def generate_latent_visualization(args):
    """Generate 2D latent space visualization."""
    # Use density estimation model
    from ngs.benchmarks.density import DensityBenchmark, DensityBenchmarkConfig
    ...
    benchmark = DensityBenchmark(config, device=args.device)
    results = benchmark.run(seed=args.seed)
    
    # Get the model
    from ngs.benchmarks.density import DensityEstimationModel
    model = DensityEstimationModel(config).to(args.device)
    
    # Re-train quickly for visualization
    from ngs.benchmarks.density import generate_moons
    train_data = generate_moons(2000, 0.05).to(args.device)
    ...
    for epoch in range(200):
        ...
        for i in range(0, len(train_data), 256):
            batch = train_data[perm[i:i+256]]
            optimizer.zero_grad()
            means, log_scales, weights = model(batch)
            from ngs.benchmarks.density import compute_nll
    ...
    with torch.no_grad():
        ...
        grid = torch.tensor(np.c_[xx.ravel(), yy.ravel()], dtype=torch.float32).to(args.device)
        
        z = model.encoder(grid)
        z_np = z.cpu().numpy()
        
        # Also encode training data
        z_train = model.encoder(train_data).cpu().numpy()
        
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    
    # Latent space grid
    axes[0].scatter(z_np[:, 0], z_np[:, 1], s=1, alpha=0.5, c='gray')
    axes[0].scatter(z_train[:, 0], z_train[:, 1], s=2, alpha=0.7, c='red')
    axes[0].set_xlabel('Latent Dim 1')
    axes[0].set_ylabel('Latent Dim 2')
    axes[0].set_title('Latent Space (Grid + Training Data)')
    axes[0].set_aspect('equal')
    
    # Routing in latent space
    routing = model.ngs.router(z)
    if isinstance(routing.indices, list):
        all_indices = torch.cat(routing.indices, dim=1).cpu().numpy()
    else:
        all_indices = routing.indices.cpu().numpy()
        
    # Color by most activated unit
    top_unit = all_indices[:, 0]
    scatter = axes[1].scatter(z_np[:, 0], z_np[:, 1], c=top_unit, s=1, cmap='tab20', alpha=0.7)
    axes[1].set_xlabel('Latent Dim 1')
    axes[1].set_ylabel('Latent Dim 2')
    axes[1].set_title('Routing Assignment in Latent Space')
    axes[1].set_aspect('equal')
    plt.colorbar(scatter, ax=axes[1])
    
    plt.tight_layout()
    plt.savefig(Path(args.output_dir) / "latent_visualization.png", dpi=150)
    plt.close()
    
    print(f"Latent visualization saved to {args.output_dir}/latent_visualization.png")
...
def main():
    ...
    print("Generating latent visualization...")
    generate_latent_visualization(args)
