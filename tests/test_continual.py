...
class TestContinualLearning:
    ...
    @pytest.mark.skip(reason="TrainConfig API mismatch - replay_size not in config")
    ...
    @pytest.mark.skip(reason="TrainConfig API mismatch")
    ...
    def test_forward_transfer(self):
        """Test forward transfer to new tasks."""
        config = NGSConfig(max_k=128, k_init=32, latent_dim=32)
        model = build_ngs(784, 10, config)
        ...
        # Train on task 0
        train_loader_0, _, _ = get_task_loaders('split_mnist', 0, 2, 64)
        trainer.train_epoch(train_loader_0)
        
        # Evaluate on unseen task 2 (forward transfer)
        _, test_loader_2, _ = get_task_loaders('split_mnist', 2, 2, 64)
        acc = evaluate_model_on_task(model, test_loader_2, trainer.config.device)
        
        # Should be better than random (0.5 for 2-class)
        assert acc > 0.4
