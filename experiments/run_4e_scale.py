"""
4E Scale: TinyShakespeare d_model=512, 32 experts, 10k steps overnight.
...
# Full config: d_model=512, 32 experts (64 max_k)
cfg = NGSConfig(
    latent_dim=512, k_init=32, max_k=64, top_k=8,
...
# Data
train_loader, val_loader, _ = create_tinyshakespeare_loaders(task_id=0, n_tasks=1, seq_len=64, batch_size=16)

opt = torch.optim.AdamW(model.parameters(), lr=3e-4)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt, 10000)

print(f"Training for 10000 steps, d_model=512, latent=512...")
model.train()
step = 0
start = time.time()
...
json.dump({'ppl': float(ppl), 'steps': step, 'time': time.time()-start},
          open('results/full/4e_scale.json', 'w'))