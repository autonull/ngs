"""
Scaled Autopoietic Splatting on CIFAR-10 (10 classes, 20 epochs).
Quick demonstration of self-organizing topology.
"""
import os
import sys
import json
...
# Load CIFAR-10 (simpler than CIFAR-100)
...
# Create model with ConvNet4CIFAR backbone + Autopoietic NGS head (smaller)
model = create_cifar_ngs(
    num_classes=10,        # CIFAR-10
    latent_dim=64,         # Smaller latent
    max_k=128,             # Much smaller max capacity
    k_init=16,
    top_k=4,
...
for epoch in range(num_epochs):
    ...
    print(f"Epoch {epoch+1:2d}: loss={avg_loss:.4f}, acc={acc:.4f}, active_K={active_k}, time={elapsed/60:.1f}min")
    if tree_stats:
...
json.dump(results, open('results/full/autopoietic_cifar10_small.json', 'w'))
print("\nResults saved to results/full/autopoietic_cifar10_small.json")