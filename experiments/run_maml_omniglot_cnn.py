"""
Full MAML training on Omniglot with CNN backbone + NGS head.
...
# Create trainer with CNN backbone
trainer = create_maml_trainer_cnn(
    num_classes=5,
    latent_dim=64,
    max_k=64,
    k_init=32,
...
backbone_params = sum(p.numel() for p in trainer.meta_model.parameters_backbone())
head_params = sum(p.numel() for p in trainer.meta_model.parameters_head())
inner_params = sum(p.numel() for p in trainer.inner_loop_params())
print(f"  CNN backbone: {backbone_params:,}")
print(f"  NGS head: {head_params:,}")
print(f"  Inner-loop params (router+codes): {inner_params:,}")
...
json.dump(results, open('results/full/maml_omniglot_cnn.json', 'w'))
print("\nResults saved to results/full/maml_omniglot_cnn.json")