"""
4A Full: DynamicHead + replay+KD + router freeze, 50 epochs/task.
"""
...
print("=" * 60)
print("4A Full: 50 epochs/task, freeze router mu, 20k replay, KD=5")
print("=" * 60)
...
for t in range(10):
    ...
    start=time.time()
    train_task(model,tr,old,epochs=50,lr=1e-3,wd=1e-4,rb=rb,rr=2.0,kw=5.0,kt=2.0,
...
json.dump({'acc':acc.tolist(),'forgetting':m.avg_forgetting,'avg_acc':m.avg_final_accuracy},
          open('results/full/4a_full50.json','w'))