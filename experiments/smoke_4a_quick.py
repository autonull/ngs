"""
Quick smoke test 4A: Split-CIFAR100 with DynamicHead - 2 tasks, 1 epoch each.
"""
import os
import sys
import json
...
def train_task(model, train_loader, old_model, epochs, lr, weight_decay, 
    ...
    """Train DynamicHead on a single task."""
...
def main():
    ...
    print(f"Quick smoke test: 2 tasks, 1 epoch each")
    print(f"{'='*60}")
    ...
    replay_buffer = ReplayBuffer(max_size=2000, seed=SEED)
    old_model = None
    ...
    print(f"\n{'='*60}")
    print("QUICK SMOKE TEST 4A RESULTS")
    print(f"{'='*60}")
    
    from experiments.metrics import print_results
    print_results(metrics, "Split-CIFAR100 + DynamicHead (quick)")
    ...
    with open('results/smoke_4a/quick_results.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)
    
    print(f"\nResults saved to results/smoke_4a/quick_results.json")
