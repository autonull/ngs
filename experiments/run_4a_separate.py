"""
4A: Completely separate linear head per class (no shared params).
Uses NGS frozen features + independent linear per class.
"""
...
# Separate linear head per class (truly independent)
...
def train_task(m, loader, epochs, lr):
    active_params = m.head.get_active_params()
    opt = torch.optim.AdamW(active_params, lr=lr)
    m.train()
...
print("=" * 60)
print("4A: Separate linear per class (no shared params)")
print("=" * 60)
...
json.dump({'acc':acc.tolist(),'forgetting':m.avg_forgetting,'avg_acc':m.avg_final_accuracy},
          open('results/full/4a_separate.json','w'))