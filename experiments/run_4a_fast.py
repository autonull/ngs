"""
Fast 4A: Split-CIFAR100 DynamicHead - minimal viable run.
1 seed, 3 epochs/task, smaller model, validates forgetting <5%.
"""
...
from ngs.core.interfaces import NGSConfig, RoutingStrategy, ParameterStorage
...
from ngs.core.interfaces import TopologyControl, MemoryManagement
cfg=NGSConfig(latent_dim=128,k_init=32,max_k=256,top_k=8,
...
rb=ReplayBuffer(2000,SEED); old=None
...
for t in range(3):
    ...
    train_task(model,tr,old,epochs=3,lr=1e-3,wd=1e-4,rb=rb,rr=1.0,kw=2.0,kt=2.0)
    for et in range(t+1):
    ...
    print(f"Task {t} done: {[f'{acc[i,t]:.3f}' for i in range(t+1)]}")

m=compute_metrics(acc,1/100)
print(f"\nForgetting: {m.avg_forgetting:.3f} (target<0.05)")
print(f"Avg Acc:    {m.avg_final_accuracy:.3f} (target>0.50 for fast)")
print("✅ PASS" if m.avg_forgetting<0.05 else "❌ FAIL")

os.makedirs('results/fast',exist_ok=True)
json.dump({'acc':acc.tolist(),'forgetting':m.avg_forgetting,'avg_acc':m.avg_final_accuracy},open('results/fast/4a.json','w'))