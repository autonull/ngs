"""
Final smoke test 4A: Split-CIFAR100 with DynamicHead - Monolithic+Direct, 3 tasks, 10 epochs, replay+KD.
"""
import os
import sys
import json
...
class BackboneDynamicHead(nn.Module):
    def __init__(self, backbone_name: str = 'resnet18', max_classes: int = 100, config=None):
        super().__init__()
...
def main():
    ...
    print(f"Final smoke test: 3 tasks, 10 epochs, Monolithic+Direct, replay+KD")
    print(f"{'='*60}")
    ...
    # Monolithic+Direct config with larger capacity
    cfg = NGSConfig(
        latent_dim=256,
        k_init=64,
        max_k=512,
    ...
    model = BackboneDynamicHead(backbone_name='resnet18', max_classes=max_classes, config=cfg).to(DEVICE)
    ...
    for task_id in range(n_tasks):
        ...
        if kd_weight > 0:
            old_model = BackboneDynamicHead(backbone_name='resnet18', max_classes=max_classes, config=cfg).to(DEVICE)
    ...
    print_results(metrics, "Split-CIFAR100 + DynamicHead (final)")
    ...
    if forgetting < 0.05 and avg_acc > 0.85:
        ...
        print("🔄 PIVOT: Need more epochs or different approach")
    ...
    results = {
        ...
        'config': {
            ...
            'ngs_config': {
                ...
                'parameter_storage': cfg.parameter_storage.value,
            },
            'device': DEVICE,
            'seed': SEED,
        }
    ...
    with open('results/smoke_4a/final_results.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)
    
    print(f"\nResults saved to results/smoke_4a/final_results.json")
