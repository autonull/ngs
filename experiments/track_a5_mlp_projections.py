...
Hypothesis: Non-linear projections fix the projection bottleneck.
...
def run_a5_experiment(config_name, layer_configs, use_mlp, mlp_multiplier, seed=42, epochs=5, device='cuda'):
    ...
    proj_type = f"MLP(x{mlp_multiplier})" if use_mlp else "Linear"
    print(f"  [{config_name}] Projections: {proj_type}")
...
def main():
    ...
    # Base layer config (progressive top_k from A1)
    base_layers = [
    ...
    sweep_configs = [
        # Baseline: linear projections, constant top_k
        {'name': 'baseline_linear_proj', 'layers': baseline_layers, 'use_mlp': False, 'mlp_mult': 1},
        # MLP projections with different multipliers on progressive top_k
        {'name': 'progressive_mlp_mult2', 'layers': base_layers, 'use_mlp': True, 'mlp_mult': 2},
        {'name': 'progressive_mlp_mult4', 'layers': base_layers, 'use_mlp': True, 'mlp_mult': 4},
        {'name': 'progressive_mlp_mult8', 'layers': base_layers, 'use_mlp': True, 'mlp_mult': 8},
        # MLP projections on baseline layers
        {'name': 'baseline_mlp_mult2', 'layers': baseline_layers, 'use_mlp': True, 'mlp_mult': 2},
        {'name': 'baseline_mlp_mult4', 'layers': baseline_layers, 'use_mlp': True, 'mlp_mult': 4},
        {'name': 'baseline_mlp_mult8', 'layers': baseline_layers, 'use_mlp': True, 'mlp_mult': 8},
        # MLP projections with growing K
        {'name': 'growing_K_mlp_mult4', 'layers': [
            {'top_k': 8, 'max_k': 16, 'latent_dim': 64},
            {'top_k': 8, 'max_k': 32, 'latent_dim': 64},
            {'top_k': 8, 'max_k': 64, 'latent_dim': 64},
            {'top_k': 8, 'max_k': 128, 'latent_dim': 64},
        ], 'use_mlp': True, 'mlp_mult': 4},
    ]
